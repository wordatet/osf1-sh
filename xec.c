				...
				{		
					...
					else if (comtype == BUILTIN)
					{
						...
						case SYSEVAL:	
							if (a1)	
								/*
					 * com[2] is actually the argument list, but we need
					 * it as a long for some legacy reason? No, it's just
					 * how it's used.
					 */
					execexp(a1, (long)&com[2]);
		...
		case TUN:
			{
				...
				loopcnt++;
				while (execbrk == 0 && (execute(whptr(t)->whtre, 0, 0, 0, 0) == 0) == 
					(type == TWH))
				{
