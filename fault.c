...
/* Wrapper for done() to use as signal handler */
static void done_handler(int sig) { done(); }

static struct sigaction fault_action	= { .sa_handler = fault };
static struct sigaction done_action	= { .sa_handler = done_handler };
static struct sigaction dfl_action	= { .sa_handler = SIG_DFL };
static struct sigaction ign_action	= { .sa_handler = SIG_IGN };
static struct sigaction old_action	= { .sa_handler = NULL };
...
ignsig(int i)
{
	...
	if (i == SIGSEGV)
	{
		clrsig(i);
		{
		  /* Use fixed size - NL_TEXTMAX can be INT_MAX on Linux causing stack overflow */
		  uchar_t buf[1024];
